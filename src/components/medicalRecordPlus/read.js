                ...
                {this.state.data.map(item=>{
                    return(
                        <div key={item.id} onClick={()=>this.popopShowing(item.id)} className="content">
                            ...
                            {((item.dataDeRetorno !== null && item.dataDeRetorno !== undefined)?<p className="secondary"><span>RETORNO:</span> {item.dataDeRetorno.split(" ",2).reduce(function(p, c){ return c+" "+p.split("-").reduce(function(p, c){ return c + "-" +p })})}</p>:"")}
                        </div>
                    );
                })}
            </List>
