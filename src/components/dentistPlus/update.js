...
class dentistPlusUpdate extends React.Component{
    state = {
        data:[]
    }
    async componentDidMount(){
        let {id} = this.props.match.params;
        let values = await API.get('/editarDentista/'+id).then((Response)=>{return Response.data});
        ...
        this.setState({data:[
            ...
            },
            {
                ...
                format:false,
                type:"binary",
                placeholder:"RESPONSÁVEL TECNICO",
                max:"255",
                value:((values[0].responsavelTecnico === true)?1:0)
            },
            {
                name:"ativo",
                option:["Sim","Não"],
    ...
    render(){
        ...
        return(
            <div>
                <Form theme={theme} data={data} title="Atualize o Dentista" url={"/atualizar/"+id} posUrl="/menu/dentistPlus" update="true"/>
            </div>
        );
    }
}
...
export default connect(state=>({theme:state.actualTheme}))(dentistPlusUpdate);